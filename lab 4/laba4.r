...
# ЧИСЛЕННОСТЬ РАБОЧЕЙ СИЛЫ (тыс. чел.) за 2017 год ----
gpclibPermit()
...
stat.Regions <- read.csv2('work_2017.csv', stringsAsFactors = F)
stat.Regions$count_2017 <- as.numeric(stat.Regions$count_2017)
...
spplot(Regions1, 'count_2017', main = 'Численность персонала',
...
# Текущие (эксплуатационные) затраты на охрану окружающей среды, включая оплату услуг природоохранного назначения, тысяча рублей за 2017 год ----
gpclibPermit()

stat.Regions <- read.csv2('environment_2017.csv', stringsAsFactors = F)
...
Regions <- Regions[grepl('^RU.MG.', Regions$HASC_2), ]
...
gp <- ggplot() +
  ...
  labs(x = 'Долгота', y = 'Широта',
       title = "Затраты на охрану окружающей среды") +
